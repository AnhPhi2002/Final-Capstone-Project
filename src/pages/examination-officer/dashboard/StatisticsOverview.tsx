...
const getBarColorByStatus = (status: string): string => {
  switch (status) {
    ...
    case "PENDING":
      return "#FF9800"; // vàng
    default:
      return "#F44336"; // đỏ cho "Not Qualified", "INACTIVE", "REJECTED", "No Group", "No Topic"
  }
};
...
const StatisticsOverview: React.FC<StatisticsOverviewProps> = ({ semesterId }) => {
  ...
  const renderBarChart = (data: StatItem[], keyField: "status" | "round") => {
    const enhancedData = data.map((item) => {
      const key = keyField === "status" ? (item as any).status : (item as any).round;
      return {
        ...item,
        fill: getBarColorByStatus(key),
      };
    });
