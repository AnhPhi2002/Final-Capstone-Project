...
export const CardSemester: React.FC<CardSemesterProps> = ({
  ...
  const filteredData = data.filter((round) => round.semesterId === selectedSemester && round.type === "TOPIC" && !round.isDeleted);
  ...
  const handleCardClick = (semesterId: string, submissionRoundId: string, roundNumber: number) => {
    navigate(`/examination/topic-list/semester/${semesterId}/submission/${submissionRoundId}/round/${roundNumber}`);
  };
