      ...
      <div className="flex flex-1 flex-col gap-4">
        {topicsLoading ? (
          ...
          filteredTopics.map((topic) => (
            <div
              key={topic.id}
              onClick={() => navigate(`/examination/topic-detail/${topic.id}/${semesterId}`)}
            ...
            >
              <Badge
                className={`${statusClasses[topic.status as "APPROVED" | "REJECTED" | "PENDING" | "IMPROVED"] || "bg-gray-100 text-gray-600 hover:bg-gray-200"} absolute top-4 right-6 px-2 py-1 rounded-md text-xs`}
              >
                {topic.status}
              </Badge>
