...
import { fetchDecisionById, updateDecision } from "@/lib/api/redux/decisionListTopc";
...
import { z } from "zod";
import { fetchDecisions } from "@/lib/api/redux/decisionListTopc"; // ✅ Đúng tên file slice của bạn
...
const UpdateDecisionListTopic: React.FC = () => {
  ...
  const { loading: decisionLoading, error: decisionError } = useSelector(
  ...
  // Fetch decision details and guidance list on mount
  useEffect(() => {
      ...
      dispatch(fetchDecisionById(decisionId))
        ...
        .then((decision) => {
          setFormData({
            ...
            type: decision.type as "DRAFT" | "FINAL",
            semesterId: decision.semesterId || semesterId,
            decisionURL: decision.decisionURL || "", // Assuming decisionURL is part of the response
          });
        })
        .catch((err) => {
          toast.error("Lỗi khi lấy thông tin quyết định", {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!decisionId) {
      toast.error("Không tìm thấy ID quyết định");
      return;
    }
    ...
    setLoading(true);
    dispatch(updateDecision({ id: decisionId, data: payload }))
  .unwrap()
  .then(() => {
    dispatch(fetchDecisions({ semesterId })); // ✅ reload lại list sau khi update
    toast.success("Cập nhật quyết định thành công");
    navigate(`/academic/decision-list-top/${formData.semesterId}`);
  })

      .catch((err) => {
        console.error("❌ Error when updating decision:", err);
      ...
      <div className="max-w-8xl mx-auto p-8">
        <Card className={`${textClass} shadow-lg`}>
          <CardContent className="p-10">
            {decisionLoading && <p>Đang tải thông tin quyết định...</p>}
            {decisionError && (
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex justify-end">
                <Button type="submit" disabled={loading || decisionLoading}>
                  {loading ? "Đang cập nhật..." : "Cập nhật quyết định"}
                </Button>
              </div>
            </form>
...
export default UpdateDecisionListTopic;