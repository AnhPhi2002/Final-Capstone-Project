...
//                 }
//             );
//             return response.data.data as CouncilDefenseAssignment;
//         } catch (error: any) {
...
export const fetchCouncilDetailForMentor = createAsyncThunk(
    ...
    async ({ councilId, semesterId }: { councilId: string; semesterId: string }, { rejectWithValue }) => {
        try {
            const response = await axiosClient.get(`/council-defense/lecturers/councils/${councilId}`, {
                params: { semesterId }
            });
            const council = response.data.data as CouncilDefense;
            if (council.isDeleted) {
    ...
    async (
      {
        ...
        email,
        role,
      }: { councilId: string; email: string; role: "council_chairman" | "council_secretary" | "council_member" },
      { rejectWithValue }
    ) => {
      try {
...
export const fetchDefenseSchedulesForStudent = createAsyncThunk(
    "councilReview/fetchDefenseSchedulesForStudent",
...
export const fetchDefenseSchedulesForMentor = createAsyncThunk(
    "councilReview/fetchDefenseSchedulesForMentor",
...
const councilDefenseSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(createDefenseSchedule.rejected, (state) => {
                state.loading = false;
                // state.error = action.payload as string;
            })
